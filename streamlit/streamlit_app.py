...
### Config
st.set_page_config(page_title="GetAround Delay", page_icon=":blue_car:", layout="wide")
...
data["delay_at_checkout_in_minutes"] = data[mask][
...
mean_per_checking_type = data.groupby(["checkin_type"])[
    "delay_at_checkout_in_minutes"
].mean()
st.write(mean_per_checking_type)

avg_checking_type_df = data.groupby(["checkin_type"])
sub_avg_checking_type_df = avg_checking_type_df[["delay_at_checkout_in_minutes"]].mean()
...
st.write("")
st.write(
    "Let's remove all entries where time_delta_with_previous_rental_in_minutes is NaN as it corresponds to a delta greater than 12 hours"
)
...
fig_pie.update_traces(hole=0.4, textinfo="label+percent")

fig_pie.update_layout(width=1200, legend=dict(font=dict(size=16)))

st.plotly_chart(fig_pie)

st.write(
    data_delta_not_null.groupby(["checkin_type", "state"])[
...
threshold = st.select_slider(
    "Choose a threshold on Connect feature only: ", options=[60, 240, 570, 720]
)
...
st.write(
    data_delta_not_null_new.groupby(["checkin_type", "state"])[
        "time_delta_with_previous_rental_in_minutes"
    ].describe()
)
...
st.write(
    ...
    diff2,
    " of problematic cases for a threshold of ",
    threshold,
    "minutes.",
)
...
st.write(
    "Delays represent ",
    ((count_delay / data.shape[0]) * 100).round(2),
    "% of total check-outs.",
)
